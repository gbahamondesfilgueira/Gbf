    ...
    IFormFile? foto)
        {
            ...
            if (existeActivo)
            {
                TempData["Error"] = "Este conductor ya está registrado en otra empresa.";
            ...
            if (!licenciasValidas.Contains(conductor.ClaseLicencia))
            {
                TempData["Error"] = "La clase de licencia no es válida.";
            ...
            TempData["Success"] = "Conductor registrado correctamente.";
        ...
        public async Task<IActionResult> ImportarCsv(IFormFile archivo, int empresaId)
        {
            ...
            foreach (var valores in registros)
            {
                try
                {
                    var nombre = valores[0].Trim();
                    var apellido = valores[1].Trim();
                    var rut = valores[2].ToUpper().Trim();
                    var fechaTexto = valores[3].Trim();
                    var licencia = valores[4].ToUpper().Trim();
                    ...
                    // 🔴 VALIDAR LICENCIA
                    if (!licenciasValidas.Contains(licencia))
                    {
                        rechazados++;
                        continue;
                    }
            ...
            TempData["Success"] = $"Carga finalizada: {agregados} conductores registrados correctamente.";
