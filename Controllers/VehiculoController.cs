...
namespace Gbf.Controllers
{
    public class VehiculoController : Controller
    {
        ...
        public IActionResult Crear(Vehiculo vehiculo)
        {
            ...
            if (existeActivo)
            {
                TempData["Error"] = "Este vehículo ya está registrado en otra empresa.";
        ...
        public async Task<IActionResult> ImportarCsv(IFormFile archivo, int empresaId)
        {
            ...
            if (errores.Any())
            {
                ...
                var empresaError = _context.Empresas.Find(empresaId);
            ...
            foreach (var valores in registros)
            {
                try
                {
                    ...
                    // 🔴 VALIDAR AÑO SEGURO
                    if (!int.TryParse(valores[5], out int anio))
                    {
                        rechazados++;
                        continue;
                    }
            ...
            // 🔥 MENSAJES
            TempData["Success"] = $"Carga finalizada: {agregados} vehículos registrados correctamente.";
